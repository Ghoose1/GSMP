...
namespace GSMP.Content.Items
{
    ...
    public class LineTest : ModItem
    {
        ...
        {
            static int NewSummonProjectile(IEntitySource source, Vector2 spawn, Vector2 velocity, int type, int rawBaseDamage, float knockback, int owner = 255, float ai0 = 0, float ai1 = 1)
            {
            ...
            if (player.ownedProjectileCounts[ModContent.ProjectileType<LineTestProj>()] < 1)
            {
                NewSummonProjectile(player.GetSource_Accessory(item), player.position, -Vector2.UnitY, ModContent.ProjectileType<LineTestProj>(), 50, 2, Main.myPlayer);
            }
        }
    ...
    //        PrepareDrawnEntityDrawing(proj, Main.player[proj.owner].cMinion);
    ...
    //        Color color = Microsoft.Xna.Framework.Color.White * proj.Opacity;
    ...
    public class LineTestProj : ModProjectile
    {
        public override string Texture => "GSMP/Assets/HallowSword";
